...
def weighted_focal_loss(y_true, y_pred):
    ...
    # Fix type mismatch for mixed precision - ensure all operations use float32
    pos_loss = tf.cast(pos_loss, tf.float32)
    pos_loss *= tf.cast(tf.reduce_sum(tf.cast(y_true[:,1] == 0, tf.float32))/(tf.reduce_sum(tf.cast(y_true[:,1], tf.float32))+K.epsilon()), tf.float32)

    # Loss for the negative examples
    neg_loss = -(p ** gamma) * tf.math.log(q)
    neg_loss = tf.cast(neg_loss, tf.float32)
...
def pre_process_ch(ch_data, fs_data, fs_resamp):
    ...
    # Use simpler, faster filtering
    # High-pass filter (0.5 Hz)
    b, a = signal.butter(2, 0.5/(fs_resamp/2), 'high')  # Reduced order from 4 to 2
    ch_data = signal.filtfilt(b, a, ch_data)

    # Low-pass filter (60 Hz) 
    b, a = signal.butter(2, 60/(fs_resamp/2), 'low')  # Reduced order from 4 to 2
    ch_data = signal.filtfilt(b, a, ch_data)

    # Notch filter (50 Hz) - only if needed
    if fs_resamp > 100:  # Only apply notch filter for higher sampling rates
        b, a = signal.butter(2, [49.5/(fs_resamp/2), 50.5/(fs_resamp/2)], 'bandstop')
        ch_data = signal.filtfilt(b, a, ch_data)

    return ch_data, fs_resamp


def apply_preprocess_multimodal(config, rec):
    """
    Preprocess all modalities (EEG, ECG, EMG, MOV) from a recording.
    
    Args:
        config: Configuration object with fs (target sampling frequency)
        rec: Data object with data, channels, and fs attributes
    
    Returns:
        list: Preprocessed channels [EEG_ch1, EEG_ch2, ECG, EMG, MOV_ch1, ..., MOV_ch12]
    """
    processed_channels = []
    
    # 1. Process EEG (2 channels: focal + cross-hemispheric)
    idx_focal = [i for i, c in enumerate(rec.channels) if c == 'BTEleft SD']
    if not idx_focal:
        idx_focal = [i for i, c in enumerate(rec.channels) if c == 'BTEright SD']
    idx_cross = [i for i, c in enumerate(rec.channels) if c == 'CROSStop SD']
    ...
    if idx_focal and idx_cross:
        ch_focal, _ = pre_process_ch(rec.data[idx_focal[0]], rec.fs[idx_focal[0]], config.fs)
        ch_cross, _ = pre_process_ch(rec.data[idx_cross[0]], rec.fs[idx_cross[0]], config.fs)
        processed_channels.extend([ch_focal, ch_cross])
    
    # 2. Process ECG (1 channel)
    idx_ecg = [i for i, c in enumerate(rec.channels) if 'ECG' in c or 'ecg' in c]
    if idx_ecg:
        ch_ecg = pre_process_ecg(rec.data[idx_ecg[0]], rec.fs[idx_ecg[0]], config.fs)
        processed_channels.append(ch_ecg)
    
    # 3. Process EMG (1 channel)
    idx_emg = [i for i, c in enumerate(rec.channels) if 'EMG' in c or 'emg' in c]
    if idx_emg:
        ch_emg = pre_process_emg(rec.data[idx_emg[0]], rec.fs[idx_emg[0]], config.fs)
        processed_channels.append(ch_emg)
    
    # 4. Process MOV (12 channels: accelerometer + gyroscope)
    # MOV channels have patterns like "SD ACC X", "SD GYR A", etc.
    idx_mov = [i for i, c in enumerate(rec.channels) if any(x in c for x in ['ACC', 'GYR', 'Acc', 'Gyr', 'acc', 'gyr'])]
    if idx_mov:
        for idx in idx_mov:
            ch_mov = pre_process_mov(rec.data[idx], rec.fs[idx], config.fs)
            processed_channels.append(ch_mov)
    
    return processed_channels


def pre_process_ecg(ch_data, fs_data, fs_resamp):
    """
    Preprocess ECG channel (heart rate variability signal).
    Simpler than EEG - focuses on R-wave detection range (0.5-40 Hz).
    """
    # Resample if needed
    if fs_resamp != fs_data:
        ch_data = signal.resample(ch_data, int(fs_resamp*len(ch_data)/fs_data))
    
    # Bandpass filter for ECG (0.5-40 Hz for R-wave detection)
    b, a = signal.butter(2, [0.5/(fs_resamp/2), 40/(fs_resamp/2)], 'bandpass')
    ch_data = signal.filtfilt(b, a, ch_data)
...
def pre_process_emg(ch_data, fs_data, fs_resamp):
    """
    Preprocess EMG channel (muscle activity signal).
    EMG typically in 20-450 Hz range, but we focus on lower frequencies for seizure-related activity.
    """
    # Resample if needed
    if fs_resamp != fs_data:
        ch_data = signal.resample(ch_data, int(fs_resamp*len(ch_data)/fs_data))
    
    # Bandpass filter for EMG (20-100 Hz for muscle activity)
    b, a = signal.butter(2, [20/(fs_resamp/2), 100/(fs_resamp/2)], 'bandpass')
    ch_data = signal.filtfilt(b, a, ch_data)
...
def pre_process_mov(ch_data, fs_data, fs_resamp):
    """
    Preprocess movement/accelerometer channel.
    Movement data is typically at lower sampling rate (25 Hz in SeizeIT2).
    We resample to match other modalities and apply light filtering.
    """
    # Resample if needed (MOV is at 25 Hz, need to upsample to 256 Hz)
    if fs_resamp != fs_data:
        ch_data = signal.resample(ch_data, int(fs_resamp*len(ch_data)/fs_data))
    
    # Low-pass filter to remove high-frequency noise (10 Hz cutoff for movement)
    # Movement patterns relevant for seizures are typically < 10 Hz
    b, a = signal.butter(2, 10/(fs_resamp/2), 'low')
    ch_data = signal.filtfilt(b, a, ch_data)
